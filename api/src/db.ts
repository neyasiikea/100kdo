...
function rowToToolkit(row: ToolkitRow): GeneratedToolkit {
  return { ...row, keywords: jsonArr(row.keywords), scenarios: jsonArr(row.scenarios), ports: jsonArr(row.ports) };
}
function jsonArr(raw: string): any[] { try { return JSON.parse(raw); } catch { return []; } }
